...
def _normalize_invoice(inv: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "sixMonthHistory" in out and isinstance(out["sixMonthHistory"], list):
        ...
        for row in out["sixMonthHistory"]:
            rr = dict(row or {})
            for k in [
                "energyKWh", "total_current_charges", "total_amount_due",
                "maximum_demand_kva", "carbonTco2e", "water_m3", "water_cost"
            ]:
                if k in rr:
...
async def health():
    return HealthResponse(
        ...
        endpoints=[
            ...
            "/api/invoices/save-to-mongodb", "/api/invoices/load-from-mongodb", "/api/invoices/mongodb-stats",
            "/ws/live-ai",
        ],
    )
...
async def get_esg_data():
    ...
    # ✅ invoices - use MongoDB if available, otherwise local storage
    if mongo_collection is not None:
        try:
            db_invoices = await mongo_collection.find({}).to_list(length=100)
            for d in db_invoices:
                d["_id"] = str(d.get("_id"))
            env["uploadedInvoiceData"] = db_invoices
            env["invoiceCount"] = len(db_invoices)
        except Exception as e:
            logger.warning(f"Failed to fetch invoices from MongoDB: {e}")
    ...
    if env.get("uploadedInvoiceData"):
        total_energy = sum((inv.get("total_energy_kwh") or 0) for inv in env["uploadedInvoiceData"])
        total_water = sum((inv.get("water_usage") or 0) for inv in env["uploadedInvoiceData"])
        total_charges = sum((inv.get("total_current_charges") or 0) for inv in env["uploadedInvoiceData"])
        estimated_co2 = total_energy * 0.99 / 1000
        
        env["aggregatedMetrics"] = {
            "totalEnergyKwh": total_energy,
            "totalWaterM3": total_water,
            "estimatedCo2Tonnes": estimated_co2,
            "totalChargesRand": total_charges,
        }
...
async def post_governance_insights(request: GovernanceInsightsRequest):
    ...
    def _fallback() -> List[str]:
        return [
            ...
            "Ethics and compliance monitoring should prioritise third-party risk and supplier adherence to codes of conduct.",
...
async def upload_invoice(file: UploadFile = File(...)):
    try:
        ...
        # ✅ auto-save to mongo if available
        if mongo_collection is not None:
...
# ✅ Best endpoint for UI population (pagination + search)
...
async def query_invoices(
    ...
    items, total = await mongo_query_invoices(q=q, company=company, page=page, page_size=page_size, sort=sort)
...
# ================== MONGODB SAVE/LOAD ENDPOINTS ==================
@app.post("/api/invoices/save-to-mongodb", tags=["Invoices"])
async def save_invoices_to_mongodb(request: MongoDBSaveRequest):
    ...
    try:
        inserted_count = 0
        for inv in invoices:
            result = await mongo_upsert_invoice(inv)
            if result:
                inserted_count += 1
        ...
        return {
            ...
            "message": f"Successfully saved {inserted_count} invoice(s) to MongoDB"
        }
    except Exception as e:
        logger.error(f"Error saving invoices to MongoDB: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to save invoices: {str(e)}")
...
async def load_invoices_from_mongodb():
    ...
    if mongo_collection is None:
        return MongoDBLoadResponse(success=False, invoices=last_invoice_summaries, count=len(last_invoice_summaries))
    
    try:
        docs = await mongo_collection.find({}).to_list(length=None)
        for d in docs:
            d["_id"] = str(d.get("_id"))
        
        return MongoDBLoadResponse(success=True, invoices=docs, count=len(docs))
    except Exception as e:
        logger.error(f"Error loading invoices from MongoDB: {e}")
...
async def get_mongodb_stats():
    ...
    if mongo_collection is None:
        total_energy = sum((inv.get("total_energy_kwh") or 0) for inv in last_invoice_summaries)
        estimated_co2 = total_energy * 0.99 / 1000
        return MongoDBStatsResponse(
            success=False,
            totalInvoices=len(last_invoice_summaries),
            totalEnergyKwh=total_energy,
            estimatedCo2=estimated_co2,
            lastUpdated=now_iso(),
            databaseName="local-memory"
        )
    
    try:
        total_invoices = await mongo_collection.count_documents({})
        
        # Calculate totals from MongoDB
        pipeline = [
            {"$group": {
                "_id": None,
                "totalEnergy": {"$sum": "$total_energy_kwh"},
                "totalWater": {"$sum": "$water_usage"},
            }}
        ]
        
        result = await mongo_collection.aggregate(pipeline).to_list(length=1)
        
        total_energy = result[0]["totalEnergy"] if result else 0
        estimated_co2 = total_energy * 0.99 / 1000 if total_energy else 0
